...
from argus_collector.web.status_code import CODE_OK

# global variables.
COLLECTORS = {}
GENERATION = 0
DEFAULT_LOG = LOG_FILE if LOG_FILE else os.path.join(os.path.dirname(__file__), 'logs',
                                                     'collector.log')
...
class Collector(object):
    ...
    def __init__(self, colname, interval, filename, mtime=0, lastspawn=0):
        ...
        # 这个字典用于跟踪并去除重复value的数据, key和value分别定义为
        #   key: (metric, tags)
        #   value: (value, repeated, line, timestamp)
        #     value: metric的上一个value
        #     repeated: 上一个value是否重复出现过
        #     line: 从采集器读取出的最后一行数据
        #     timestamp: value第一次出现的时间戳
        self.values = {}
        self.lines_sent = 0
    ...
    def read(self):
        ...
        try:
            ...
            if out:
                LOG.error('reading %s got %d bytes on stderr: \n' % (self.name, len(out)), exc_info=True)
...
class StdinCollector(Collector):
    ...
    def __init__(self):
        ...
        # except as a tests in the stdin mode.
        self.proc = True
...
class ReaderThread(threading.Thread):
    ...
    def process_line(self, col, line):
        ...
        # ToDo: keep float timestamp? This means modify log output format...
        if FLOAT_TIMESTAMP:
        ...
        if self.dedupinterval != 0:  # if 0 we do not use dedup
            ...
            if key in col.values:
                ...
                if timestamp <= col.values[key][3]:
                    ...
                    LOG.error("Timestamp is too far out in the future: metric=%s%s"
                              " old_ts=%, new_ts=%d - ignoring data point"
                              " (value=%r, collector=%s)", metric, tags,
                              col.values[key][3], timestamp, value, col.name)
                    return
            ...
            # tags (essentially the same as what TSD uses for the row key).
...
def setup_logging(logfile=DEFAULT_LOG, loglevel=DEFAULT_LOG_LEVEL, max_bytes=None, backup_count=None):
    """Sets up logging and associated handlers."""

    _loglevel = eval('logging.{0}'.format(loglevel.upper()))
...
def parse_cmdline(argv):
    ...
    try:
        from argus_collector.collectors.etc import config
        defaults = config.get_defaults()
    except ImportError:
        sys.stderr.write("ImportError: Could not load defaults from configuration.")
    except:
...
def setup_python_path():
    ...
    if pythonpath:
        pythonpath += ':'
    pythonpath += os.path.dirname(BASE_DIR)
    os.environ['PYTHONPATH'] = pythonpath
    LOG.debug('Set PYTHONPATH to %r', pythonpath)
    sys.path.insert(0, os.path.dirname(BASE_DIR))


def check_fileformat(cdir):
    """
    确保采集器目录下的脚本均为unix文件格式
    :param cdir: 采集器目录
    :return:
    """
    LOG.info('Checking fileformat...')
    changed_list = []
    for f in os.listdir(cdir):
        if str(f).isdigit():
            _f = os.path.join(cdir, f)
            if os.path.isdir(_f):
                for each in os.listdir(_f):
                    _each = os.path.join(_f, each)
                    changed = dos2unix(_each)
                    if changed:
                        changed_list.append(_each)
    if changed_list:
        LOG.info('Files convert from dos to unix: {0}'.format(changed_list))


def dos2unix(dos_file, unix_file=None):
    """
    :param dos_file:
    :param unix_file:
    :return: True/False, whether the file is changed
    """
    if unix_file is None:
        unix_file = dos_file
    with open(dos_file) as f:
        dos_lines = f.readlines()
    unix_lines = [line[:-2] + '\n' if line.endswith('\r\n') else line for line in dos_lines]
    flag_changed = ''.join(unix_lines) != ''.join(dos_lines)
    if flag_changed:
        with open(unix_file, 'w') as f:
            f.write(''.join(unix_lines))
    return flag_changed


def clear_collectors_data():
    """清空collectors/data/目录下的临时数据"""
    LOG.info('Clear temporary data for scripts...')
    data_dir = os.path.join(BASE_DIR, 'collectors', 'data')
    removed_files = []
    for data_file in os.listdir(data_dir):
        data_path = os.path.join(data_dir, data_file)
        os.remove(data_path)
        removed_files.append(data_path)
    if removed_files:
        LOG.debug('Data files removed: {0}'.format(removed_files))


def check_permission(cdir):
    """检查采集脚本的权限是否OK，确保当前用户有执行权限"""
    LOG.info('Check collectors\' permission...')
    for f in os.listdir(cdir):
        if str(f).isdigit():
            f_path = os.path.join(cdir, f)
            if os.path.isdir(f_path):
                for c in os.listdir(f_path):
                    c_path = os.path.join(f_path, c)
                    if os.stat(c_path).st_mode & stat.S_IEXEC == 0:
                        os.chmod(c_path, os.stat(c_path).st_mode | stat.S_IEXEC)
                        LOG.debug('Exec permission added for collector: {0}'.format(c_path))


def push_tcollector_status():
    """"""
    LOG.debug('Pushing tcollector status to agent manager')
    while True:
        host = socket.gethostname()
        tsd_host = OPEN_TSDB_HOST
        tsd_port = OPEN_TSDB_PORT
        collectors = get_collectors()
        uptime = get_uptime()

        _data = {}
        _data['host'] = host
        _data['tsd_host'] = tsd_host
        _data['tsd_port'] = tsd_port
        _data['collectors'] = collectors
        _data['uptime'] = uptime
        # import pdb
        # pdb.set_trace()
...
def get_collectors():
    """
    Find out all the collectors in digit named subdirectory in collectors.
    """
    coldir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'collectors')
    collectors = {}
...
def get_uptime():
    pid = ''
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'collector.pid')) as pid_file:
...
def main(argv):
    ...
    setup_logging(options.logfile, options.loglevel, options.max_bytes or None,
    ...
    # check file formats(dos2unix)
    check_fileformat(options.cdir)
    # clear collectors/data/
    clear_collectors_data()
    # check exec permission
    check_permission(options.cdir)
...
def spawn_children():
    ...
    LOG.debug('Start to spawn collectors...')
    if not ALIVE:
        LOG.debug('Not ALIVE!')
        return
...
def populate_collectors(coldir):
    ...
    LOG.debug('populating collectos...')
    global GENERATION
    GENERATION += 1
